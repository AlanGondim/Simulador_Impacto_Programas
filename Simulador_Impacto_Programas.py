...
# --- MOTOR DE BANCO DE DADOS (Configuração de Persistência Cloud) ---
def init_db():
    # URL explícita para evitar que o Streamlit se perca no diretório temporário
...
if aba == "Nova Análise":
    ...
    # Gráfico Seaborn com Rótulos de Dados
    fig, ax = plt.subplots(figsize=(10, 5))
    plot_data = pd.DataFrame({
        'Cenário': ['Original', 'Original', 'Impactado', 'Impactado'],
        'Métrica': ['Valor Contrato', 'Lucro Líquido', 'Valor Contrato', 'Lucro Líquido'],
        'Valor (R$)': [v_proj, lucro_orig, v_final, novo_lucro]
    })
    cor_lucro = "#B24B22" if n_margem >= 10 else "#FF0000"
    sns.barplot(data=plot_data, x='Cenário', y='Valor (R$)', hue='Métrica', palette=["#1B6DBE", cor_lucro], ax=ax)
